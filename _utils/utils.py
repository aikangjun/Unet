...
def cvt2RGB(image):
    # 传入通过Image.open() 读取的文件
    # 判断图片是否为RGB格式，进行转换
    # 该函数用于检测时，对不同格式图片进行转换
    if np.shape(image).__len__() == 3 and np.shape(image)[-1] == 3:
        return image
    else:
...
class WarmUpCosineDecayScheduler:
    ...
    def batch_begin(self):
        ...
        return lr
def calculate_score(y_trues, y_preds, depth: int = 21):
    # 计算f1_score

    scores = []
    true_class = tf.cast(tf.argmax(y_trues,axis=-1), dtype=tf.int8)
    pred_class = tf.cast(tf.argmax(y_preds, axis=-1), dtype=tf.int8)

    for i in range(depth):

        precision_num = tf.reduce_sum(tf.cast(tf.equal(pred_class, i), dtype=tf.float32))

        true_bool_mask = tf.equal(true_class, i)
        pred_bool_mask = tf.equal(pred_class, i)

        bool_mask = tf.reduce_all(tf.stack([true_bool_mask, pred_bool_mask], axis=-1), axis=-1)
        true_positive_num = tf.reduce_sum(tf.cast(bool_mask, dtype=tf.float32))

        false_negative_num = tf.reduce_sum(tf.cast(true_bool_mask, dtype=tf.float32)) - true_positive_num
        recall_num = true_positive_num + false_negative_num

        precision = true_positive_num / precision_num if precision_num > 0 else 0.
        recall = true_positive_num / recall_num if recall_num > 0 else 0.
        if precision > 0 or recall > 0:
            score = 2 * (precision * recall) / (precision + recall)
            scores.append(score)
        if precision_num > 0 and recall_num > 0 and true_positive_num == 0:
            scores.append(0.)

    f1_score = tf.square(tf.reduce_mean(scores))

    return tf.zeros(shape=()) if tf.math.is_nan(f1_score) else f1_score